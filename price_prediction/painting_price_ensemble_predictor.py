...
class PaintingPriceEnsemblePredictor:
    """
    ...
    def filter_prediction_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if numeric_defaults:
            df = pd.concat([df, pd.DataFrame(numeric_defaults, index=df.index)], axis=1)

        if text_defaults:
            df = pd.concat([df, pd.DataFrame(text_defaults, index=df.index)], axis=1)

        # Track additional columns for processing later
        self.additional_numeric_columns.extend(missing_numeric_cols)
        self.additional_text_columns.extend(missing_text_cols)
    ...
    def predict_with_test_split(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Preprocesses data, performs train-test split, trains regressors, and evaluates performance.
        """
        for regressor in self.regressors:
            regressor.clear_fit()
